...
public class ControllerLogic implements Serializable{
	...
	public boolean addSlot(Calendar startingTime, int productionPlanSerial) {
         ...
         */
        public boolean addUser(String username, String password, E_UserType type,E_SecretQuestion secretQuestion, String secretAnswer) {
		boolean flag = model.addUser(username, password,type, secretQuestion, secretAnswer);
		if (flag)
			MyFileLogWriter.writeToLogFile("User added: " + username, true);
		else
			MyFileLogWriter.writeToLogFile("User wasn't added", true);
		return flag;
	}
	/**
	...
	public boolean removeShift(Calendar startingTime) {
         ...
         * remove Slot
         */
        public boolean removeSlot(Calendar startingTime) {
		boolean flag = model.removeSlot(startingTime);
		if (flag)
			MyFileLogWriter.writeToLogFile("Slot removed: " + startingTime.getTime().toString(), true);
		else
			MyFileLogWriter.writeToLogFile("Slot wasn't removed", true);
		return flag;
	}
        /**
         * Remove a user
		...
		boolean flag = model.removeUser(username);
		if (flag)
			MyFileLogWriter.writeToLogFile("User removed: " + username, true);
		else
			MyFileLogWriter.writeToLogFile("User wasn't removed", true);
		return flag;
	}
    ...
    /**
	 * update manufacturer in the system
	 * @param name Manufacturer name
	 * @param country Address country
	 * @param city Address city
	 * @param street Address street
	 ...
	 * @return True if added successfully
	 */
	public boolean updateManufacturer(String name, String country, String city, String street,
			int houseNumber, String zipCode, double profitPercentage) {
		boolean flag = model.updateManufacturer(name, country, city, street, houseNumber, zipCode, profitPercentage);
		if (flag)
			MyFileLogWriter.writeToLogFile("Manufacturer updated: " + name, true);
		else
			MyFileLogWriter.writeToLogFile("Manufacturer wasn't updated", true);
		return flag;
	}
	/**
	 ...
	 * @return True if added successfully
	 */
	public boolean updateWarehouse(int warehouseNumber, String country, String city, String street,
			int houseNumber, String zipCode, int maxPartsStored, int maxVehiclesStored) {
		boolean flag = model.updateWarehouse(warehouseNumber, country, city, street, houseNumber, zipCode, maxPartsStored, maxVehiclesStored);
		if (flag)
			MyFileLogWriter.writeToLogFile("Warehouse updated: " + warehouseNumber, true);
		else
			MyFileLogWriter.writeToLogFile("Warehouse wasn't updated", true);
		return flag;
	}
	/**
	 ...
	 * @return True if added successfully
	 */
	public boolean updateProductionPlanType(int serialNumber, int numberOfSlots) {
		boolean flag = model.updateProductionPlanType(serialNumber, numberOfSlots);
		if (flag)
			MyFileLogWriter.writeToLogFile("Production plan type updated: " + serialNumber, true);
		else
			MyFileLogWriter.writeToLogFile("Production plan type wasn't updated", true);
		return flag;
	}
	/**
	 ...
	 * @return True if added successfully
	 */
	public boolean updateProductionPlan(int serialNumber, int typeSerialNumber) {
		boolean flag = model.updateProductionPlan(serialNumber, typeSerialNumber);
		if (flag)
			MyFileLogWriter.writeToLogFile("Production plan updated: " + serialNumber, true);
		else
			MyFileLogWriter.writeToLogFile("Production plan wasn't updated", true);
		return flag;
	}
	/**
	 * update Vehicle model in the system
	 * Don't forget to retrieve the production plan type of this model
	 * @param productionPlanTypeSerial Serial number of the production plan needed to build this vehicle
	 * @param modelName Name of this model
	 * @param vehicleType Type of this model
	 ...
	 * @see E_VehicleType
	 */
	public boolean updateVehicleModel(int productionPlanTypeSerial, String modelName,
			E_VehicleType vehicleType, int hp, int topSpeed, int numOfSeats,
			boolean isAutomatic, double productionCost, Calendar enteredProduction) {
		boolean flag = model.updateVehicleModel(productionPlanTypeSerial, modelName, vehicleType, hp, topSpeed, numOfSeats, isAutomatic, productionCost, enteredProduction);
		if (flag)
			MyFileLogWriter.writeToLogFile("Vehicle model updated: " + modelName, true);
		else
			MyFileLogWriter.writeToLogFile("Vehicle model wasn't updated", true);
		return flag;
	}
	/**
	 * update part type in the system
	 * @param partNumber Part type serial number
	 * @param partName Name of the part
	 * @param category Category of the part
	 * @param productionCost Cost to produce this part
	 ...
	 * @see E_PartCategory
	 */
	public boolean updatePartType(int partNumber, String partName, E_PartCategory category, double productionCost) {
		boolean flag = model.updatePartType(partNumber, partName, category, productionCost);
		if (flag)
			MyFileLogWriter.writeToLogFile("Part type updated: " + partNumber, true);
		else
			MyFileLogWriter.writeToLogFile("Part type wasn't updated", true);
		return flag;
	}
	/**
	 * update order in the system
	 * Don't forget to retrieve the manufacturer
	 * @param orderNumber Order number
	 * @param orderedFrom Ordering side
	 * @param orderedTo receiving side
	 ...
	 * @return True if added successfully
	 */
	public boolean updateOrder(int orderNumber, String orderedFrom, String orderedTo, Calendar dueDate) {
		boolean flag = model.updateOrder(orderNumber, orderedFrom, orderedTo, dueDate);
		if (flag)
			MyFileLogWriter.writeToLogFile("Order updated: " + orderNumber, true);
		else
			MyFileLogWriter.writeToLogFile("Order wasn't updated", true);
		return flag;
	}
	/**
	 ...
	 * @return True if added successfully
	 */
	public boolean updateVehicle(String vin, String modelName, int productionPlanSerial, int orderNumber) {
		boolean flag = model.updateVehicle(vin, modelName, productionPlanSerial, orderNumber);
		if (flag)
			MyFileLogWriter.writeToLogFile("Vehicle updated: " + vin, true);
		else
			MyFileLogWriter.writeToLogFile("Vehicle wasn't updated", true);
		return flag;
	}
	/**
	 * update a work station in the system
	 * @param stationNumber Work station number
	 * @param cat Part category of the work station
	 * @return True if added successfully
	 */
	public boolean updateWorkStation(int stationNumber, E_PartCategory cat) {
		boolean flag = model.updateWorkStation(stationNumber, cat);
		if (flag)
			MyFileLogWriter.writeToLogFile("Work station updated: " + stationNumber, true);
		else
			MyFileLogWriter.writeToLogFile("Work station wasn't updated", true);
		return flag;
	}
	/**
	 ...
	 * @return True if added successfully
	 */
	public boolean updateGuard(String id, String name, String country, String city, String street,
			int houseNumber, String zipCode, Calendar birthday, Calendar employmentDate,
			double monthlySalary, int warehouseNumber) {
		boolean flag = model.updateGuard(id, name, country, city, street, houseNumber, zipCode, birthday, employmentDate, monthlySalary, warehouseNumber);
		if (flag)
			MyFileLogWriter.writeToLogFile("Guard updated: " + id, true);
		else
			MyFileLogWriter.writeToLogFile("Guard wasn't updated", true);
		return flag;
	}
	/**
	 ...
	 * @return True if added successfully
	 */
	public boolean updateShiftManager(String id, String name, String country, String city, String street,
			int houseNumber, String zipCode, Calendar birthday, Calendar employmentDate,
			double hourlyWage, double extraWage) {
		boolean flag = model.updateShiftManager(id, name, country, city, street, houseNumber, zipCode, birthday, employmentDate, hourlyWage, extraWage);
		if (flag)
			MyFileLogWriter.writeToLogFile("Shift manager updated: " + id, true);
		else
			MyFileLogWriter.writeToLogFile("Shift manager wasn't updated", true);
		return flag;
	}
	/**
	 * updates a worker
	 ...
	 * @return True if added successfully
	 */
	public boolean updateWorker(String id, String name, String country, String city, String street,
			int houseNumber, String zipCode, Calendar birthday, Calendar employmentDate,
			double hourlyWage) {
		boolean flag = model.updateWorker(id, name, country, city, street, houseNumber, zipCode, birthday, employmentDate, hourlyWage);
		if (flag)
			MyFileLogWriter.writeToLogFile("Worker updated: " + id, true);
		else
			MyFileLogWriter.writeToLogFile("Worker wasn't updated", true);
		return flag;
	}
	/**
	 * updates shift in the system
	 * @param startingTime Starting time
	 * @param ShiftManagerId Manager for this shift
	 * @return True if added successfully
	 */
	public boolean updateShift(Calendar startingTime, String ShiftManagerId) {
		boolean flag = model.updateShift(startingTime, ShiftManagerId);
		if (flag)
			MyFileLogWriter.writeToLogFile("Shift updated: " + startingTime.getTime().toString(), true);
		else
			MyFileLogWriter.writeToLogFile("Shift wasn't updated", true);
		return flag;
	}
	/**
	 * update a slot
	 ...
	 * @return True if added successfully
	 */
	public boolean updateSlot(Calendar startingTime, int productionPlanSerial) {
		boolean flag = model.updateSlot(startingTime, productionPlanSerial);
		if (flag)
			MyFileLogWriter.writeToLogFile("Slot updated: " + startingTime.getTime().toString(), true);
		else
			MyFileLogWriter.writeToLogFile("Slot wasn't updated", true);
		return flag;
	}
    /**
     ...
     * @return 
     */
    public boolean updateUser(String username, String password, E_UserType type,E_SecretQuestion secretQuestion, String secretAnswer) {
		boolean flag = model.updateUser(username, password,type, secretQuestion, secretAnswer);
		if (flag)
			MyFileLogWriter.writeToLogFile("User updated: " + username, true);
		else
			MyFileLogWriter.writeToLogFile("User wasn't updated", true);
		return flag;
	}
    /**
     * export data to .ser file
     * @param fileName
     * @return
     * @throws IOException 
     */
    public boolean serialization (String fileName) throws IOException {
		boolean flag = model.serialization(fileName);
		if (flag)
    ...
    /**
     * import data from .ser file
     * @param fileName
     * @return
     * @throws IOException 
     */
    public boolean deserialization (String fileName) throws IOException {
		boolean flag = model.deserialization(fileName);
		if (flag)
